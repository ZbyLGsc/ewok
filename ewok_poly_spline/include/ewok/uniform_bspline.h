...
public:
  ...
  _Scalar evaluate(_Scalar t, int derivative, int& s_i) const
  {
    ...
    {
      res = res * u + coefficients[i] * Polynomial<_N, _Scalar>::base_coefficients_(derivative, i);
    }
  ...
  _Scalar evaluateWithControlPointsGrad(_Scalar t, int derivative, int& grad_start_idx, VectorNT& grad) const
  {
    _Scalar s = t / dt_;
    int s_i = s;
    _Scalar u = s - s_i;
  ...
  _Scalar quadraticCost(int deriv, int cp_start_idx = 0, int num_points = -1) const
  {
    return quadraticCost(quadratic_cost_jacobian[deriv - 1] * pow_inv_dt_[2 * deriv - 1], cp_start_idx, num_points);
  }

  _Scalar quadraticCost(const MatrixN& quadratic_cost, int cp_start_idx = 0, int num_points = -1) const
  {
    if (control_points_.size() < _N)
      return 0;

    if (num_points < 0)
      num_points = control_points_.size();
    ...
    for (int idx = min_idx; idx < max_idx; ++idx)
    {
      ...
      _Scalar cost = coefficients.transpose() * quadratic_cost * coefficients;
      res += cost;
    }
  ...
  _Scalar quadraticCostWithGrad(int deriv, std::vector<_Scalar>& cp_grad, int cp_start_idx = 0,
                                int num_points = -1) const
  {
    return quadraticCostWithGrad(quadratic_cost_jacobian[deriv - 1] * pow_inv_dt_[2 * deriv - 1], cp_grad, cp_start_idx,
                                 num_points);
  }

  _Scalar quadraticCostWithGrad(const MatrixN& quadratic_cost, std::vector<_Scalar>& cp_grad, int cp_start_idx = 0,
                                int num_points = -1) const
  {
    if (control_points_.size() < _N)
      return 0;
  ...
  inline void setControlPointsData(const std::vector<double>& data, int cp_start_idx, int data_start_idx,
                                   int num_points)
  {
  ...
  inline void getControlPointsData(std::vector<double>& data, int cp_start_idx, int data_start_idx,
...
typename UniformBSpline<_N, _Scalar>::MatrixNArray computeQuadraticCostJacobian()
{
  ...
  typename UniformBSpline<_N, double>::MatrixNArray quadratic_coefficients = computeQuadraticCoefficients<_N, double>();

  for (int i = 0; i < 4; ++i)
  {
    typename UniformBSpline<_N, double>::MatrixN m;
    m = 0.5 * blending_matrix.transpose() * quadratic_coefficients[i] * blending_matrix;
...
const typename UniformBSpline<_N, _Scalar>::MatrixN
    UniformBSpline<_N, _Scalar>::blending_matrix = computeBlendingMatrix<_N, _Scalar>();

template <int _N, typename _Scalar>
const typename UniformBSpline<_N, _Scalar>::MatrixNArray
    UniformBSpline<_N, _Scalar>::quadratic_cost_jacobian = computeQuadraticCostJacobian<_N, _Scalar>();
