...
public:
  ...
  void setNumCollisionChecksPerSegment(int n)
  {
    for (int k = 0; k < 5; k++)
    {
      ...
      for (int i = 0; i < n; i++)
      {
        ...
        int grad_start_idx;
        spline_.evaluateWithControlPointsGrad(time, k, grad_start_idx, segment_grads[k][i]);
      }
    }
  }
  ...
  void setDistanceBuffer(EuclideanDistanceRingBuffer<9>::Ptr& edrb)
  {
    edrb_ = edrb;
  }
  ...
  void getMarkers(visualization_msgs::MarkerArray& traj_marker, const std::string& ns = "spline_opitimization_markers",
                  const Vector3& color1 = Vector3(0, 1, 0), const Vector3& color2 = Vector3(0, 1, 1))
  {
  ...
  double getNumericCollisionErrorGrad(std::vector<double>& grad) const
  {
    ...
    for (int i = 0; i < 3; i++)
    {
      for (int j = 0; j < num_cp_opt; j++)
      {
        ...
        current_spline.coeff(i, cp_opt_start_idx + j) += delta;
        grad[i * num_cp_opt + j] = (collisionError(current_spline, 1.0, tmp) - value) / delta;
      }
    }
...
protected:
  ...
  {
    return reinterpret_cast<UniformBSpline3DOptimization*>(data)->combinedError(x, grad);
  }

  static double wrapTrajectoryTime(const std::vector<double>& x, std::vector<double>& grad, void* data)
  {
    return reinterpret_cast<UniformBSpline3DOptimization*>(data)->closestTimeError(x, grad);
  }
  ...
  void setDefaultWeights()
  {
    ...
    endpoint_error_weights = Vector3(1e5, 1e5, 0);
  ...
  _Scalar closestTimeError(const std::vector<double>& x, std::vector<double>& grad)
  {
  ...
  double combinedError(const std::vector<double>& x, std::vector<double>& grad)
  {
    UniformBSpline3D<_N, _Scalar> current_spline(spline_);
  ...
  double quadraticCostError(const UniformBSpline3D<_N, _Scalar>& current_spline, double lambda,
                            std::vector<double>& grad) const
  {
    ...
    else
    {
      ...
      _Scalar value =
          current_spline.quadraticCostWithGrad(quadratic_cost_matrix, cur_grad, cp_opt_start_idx, num_cp_opt);
  ...
  double endpointError(const UniformBSpline3D<_N, _Scalar>& current_spline, int derivative, double lambda,
                       std::vector<double>& grad) const
  {
  ...
  double collisionError(const UniformBSpline3D<_N, _Scalar>& current_spline, double lambda,
                        std::vector<double>& grad) const
  {
    if (!edrb_.get())
      return 0;
        ...
        if (!grad.empty())
        {
          ...
          for (int i = 0; i < _N; i++)
          {
            ...
            if (current_idx >= cp_opt_start_idx && current_idx < (cp_opt_start_idx + num_cp_opt))
            {
              ...
              grad[0 * num_cp_opt + idx] += lambda * (diff / distance_threshold_) * grad_p[0] * segment_grads[0][k][i];
              grad[1 * num_cp_opt + idx] += lambda * (diff / distance_threshold_) * grad_p[1] * segment_grads[0][k][i];
              grad[2 * num_cp_opt + idx] += lambda * (diff / distance_threshold_) * grad_p[2] * segment_grads[0][k][i];
            }
          }
        }
      }
  ...
  double softLimitError(const UniformBSpline3D<_N, _Scalar>& current_spline, int derivative, double lambda,
                        std::vector<double>& grad) const
  {
    double total_error = 0;
    ...
    for (int segment_idx = start_segment_idx; segment_idx < end_segment_idx; segment_idx++)
    {
      for (int k = 0; k < segment_grads[derivative].size(); k++)
      {
        ...
        if (norm2 <= limits2_[derivative - 1])
          continue;
        ...
        if (!grad.empty())
        {
          ...
          for (int i = 0; i < _N; i++)
          {
            ...
            if (current_idx >= cp_opt_start_idx && current_idx < (cp_opt_start_idx + num_cp_opt))
            {
              ...
              grad[0 * num_cp_opt + idx] += lambda * 2 * point[0] * error * segment_grads[derivative][k][i];
              grad[1 * num_cp_opt + idx] += lambda * 2 * point[1] * error * segment_grads[derivative][k][i];
              grad[2 * num_cp_opt + idx] += lambda * 2 * point[2] * error * segment_grads[derivative][k][i];
            }
          }
        }
      }
...
public:
  ...
  EuclideanDistanceRingBuffer<9>::Ptr edrb_;
